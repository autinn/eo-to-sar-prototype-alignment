"""Fine-tune DINOv3 with unpaired EO to SAR MMD alignment."""
...
# Hyperparameters used for the unpaired global-MMD experiment.
...
# Balance one modality without pairing its samples with the other modality.
def make_weighted_sampler(labels: list[int], seed: int) -> WeightedRandomSampler:
    """Create an inverse-frequency sampler with its own random generator."""
...
# Train one SAR classifier while matching its global feature distribution to EO.
def train_one_seed(
    seed: int,
    train_sar: ImageFolder,
    train_eo: ImageFolder,
    ...
    """Train one unpaired MMD model and retain only its best test predictions."""
    seed_everything(seed)

    # The SAR backbone receives LoRA adapters. The EO backbone stays fully frozen.
    sar_backbone = load_dino_model(dino_repo, dino_weights)
    eo_backbone = load_dino_model(dino_repo, dino_weights).to(device)
    for parameter in sar_backbone.parameters():
        parameter.requires_grad = False
    for parameter in eo_backbone.parameters():
        parameter.requires_grad = False
    eo_backbone.eval()

    model = DINOClassifier(sar_backbone, num_classes=len(class_names))
    model = apply_lora(
        model,
        rank=LORA_RANK,
        alpha=LORA_ALPHA,
    ...
    # Different seeds make the weighted EO and SAR sample orders independent.
    sar_loader = DataLoader(
        train_sar,
    ...
    eo_loader = DataLoader(
        train_eo,
        batch_size=BATCH_SIZE,
        sampler=make_weighted_sampler(train_eo.targets, seed + 1),
        num_workers=NUM_WORKERS,
        pin_memory=device.type == "cuda",
    )
    ...
    alignment_loss = MMDLoss().to(device)
    ...
    # Each SAR batch is matched to an independently sampled, unpaired EO batch.
    for epoch in range(1, EPOCHS + 1):
        ...
        for (sar_images, labels), (eo_images, _) in zip(sar_loader, eo_loader):
            sar_images = sar_images.to(device, non_blocking=True)
            eo_images = eo_images.to(device, non_blocking=True)
            labels = labels.to(device, non_blocking=True)

            optimizer.zero_grad()
            logits, sar_features = model(sar_images)
            with torch.no_grad():
                eo_features = F.normalize(eo_backbone(eo_images), dim=1)

            loss_classification = classification_loss(logits, labels)
            loss_alignment = alignment_loss(sar_features, eo_features)
            loss = (
        ...
        print(
            ...
            f"MMD {total_alignment_loss / num_batches:.4f} | "
    ...
    save_json(
        {
            ...
            "settings": {
                ...
                "mmd_kernel": "multi_bandwidth_rbf",
                "mmd_bandwidth_estimator": "median",
                "lora_rank": LORA_RANK,
...
# Run five independent unpaired EO to SAR MMD alignment runs.
def main() -> None:
    ...
    output_dir = paths["output_dir"] / "mmd_alignment"
